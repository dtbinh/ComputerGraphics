...
bool Mesh::triangleIntersectTest(Ray ray, MeshTriangle meshTriangle, real_t t0, real_t t1, HitRecord *hitRecord, Vector2& texCoord) const{
	real_t gamma, beta, t, mag;
	real_t a, b, c, d, e, f, g, h, i, j, k, l;
	a = vertices[meshTriangle.vertices[0]].position.x - vertices[meshTriangle.vertices[1]].position.x;
	b = vertices[meshTriangle.vertices[0]].position.y - vertices[meshTriangle.vertices[1]].position.y;
	c = vertices[meshTriangle.vertices[0]].position.z - vertices[meshTriangle.vertices[1]].position.z;

	d = vertices[meshTriangle.vertices[0]].position.x - vertices[meshTriangle.vertices[2]].position.x;
	e = vertices[meshTriangle.vertices[0]].position.y - vertices[meshTriangle.vertices[2]].position.y;
	f = vertices[meshTriangle.vertices[0]].position.z - vertices[meshTriangle.vertices[2]].position.z;

	g = ray.rayDirection.x;
	h = ray.rayDirection.y;
	i = ray.rayDirection.z;

	j = vertices[meshTriangle.vertices[0]].position.x - ray.eyePosition.x;
	k = vertices[meshTriangle.vertices[0]].position.y - ray.eyePosition.y;
	l = vertices[meshTriangle.vertices[0]].position.z - ray.eyePosition.z;
	...
	hitRecord->time = t;
	hitRecord->normal = (1-beta-gamma) * vertices[meshTriangle.vertices[0]].normal + beta * vertices[meshTriangle.vertices[1]].normal + gamma * vertices[meshTriangle.vertices[2]].normal;
	
	texCoord = ( 1-beta - gamma) * vertices[meshTriangle.vertices[0]].tex_coord + beta * vertices[meshTriangle.vertices[1]].tex_coord
				+gamma * vertices[meshTriangle.vertices[2]].tex_coord;
	
	return true;
}

Vector3 Mesh::getMax(MeshTriangle meshTriangle) const{
	return vmax(vmax(vertices[meshTriangle.vertices[0]].position, vertices[meshTriangle.vertices[1]].position), vertices[meshTriangle.vertices[2]].position);
}

Vector3 Mesh::getMin(MeshTriangle meshTriangle) const{
	return vmin(vmin(vertices[meshTriangle.vertices[0]].position, vertices[meshTriangle.vertices[1]].position), vertices[meshTriangle.vertices[2]].position);
}
