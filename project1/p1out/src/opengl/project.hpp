...
public:
	...
	//change the status of water updating
	void modifyWaterStatus(bool updateStatus);
	//change the line/colorfill status
	void modifyMesh(bool updateMesh);
	//change the transparency of the water
	void modifyTransparency(bool updateTrans);
private:
	...
	//members
	Vector3* heightmapMeshVertice; //array for heightmap vertice
	Triangle* heightmapMeshTriangle; //array for heightmap triangle index
	Vector3* meshNormal; //array for mesh normal vectors
	Vector3* heightmapMeshNormal; //array for heightmap normal
	int meshList; //list name for pool mesh
	Vector3 cam_pos, cam_ori, cam_up; //camera related
	double delta; 
	int num_heightmap_triangles, num_heightmap_vertice;
	int SIZE; 
	bool isWaterUpdated, isMeshOnly, isTransparent;
